...
import gmail.antonsmirnov3006.selenium.widget.*;
...
public class TinkoffMobilePage extends AbstractBasePage {
    ...
    public void compareDefaultPrices() {
        ...
        logger.info("Выбрали регион - Москва");
        String priceMoscow = priceInTotal.getText();
        logger.info("Общая цена по Москве: {}", priceMoscow);
        selectRegionOnPopup("Краснодар");
        logger.info("Выбрали регион - Краснодар");
        String priceKrasnodar = priceInTotal.getText();
        logger.info("Общая цена по Краснодару: {}", priceKrasnodar);
    ...
    public void compareMaxPrices() {
        ...
        logger.info("Общая цена по Москве: {}", maxPriceMoscow);
        ...
        logger.info("Общая цена по Краснодару: {}", maxPriceKrasnodar);
    ...
    public void checkPriceInTotal(String text) {
        ...
        softly.assertAll();
        logger.info("Проверили, что общая цена = {}", text);
    }

    public void checkOrderButtonIsClickable() {
        softly.assertThat(orderButton.isClickable()).isTrue();
        softly.assertAll();
        logger.info("Проверили, что кнопка Заказа сим-карты активна");
    ...
    public void checkTabName(String tabName) {
        ...
        logger.info("Проверили, что название вкладки = {}", tabName);
    }
